...
async function handleAiProfileAction(db, req, actor) {
  const action = String(req.body?.action || '').trim();
  await requireAdminPermission(req, action === 'ai-profile:generate' ? 'ai:update' : 'ai:read');
  const resource = String(req.body?.resource || req.body?.type || '').trim();
  ...
  await writeAuditLog(db, req, actor, action, config.collection, id, { label: `Сгенерирован AI Profile: ${entity.name || id}`, status: aiProfile.status, missingFields: aiProfile.missingFields });
...
async function handleAutomationAction(db, req, actor) {
  const action = String(req.body?.action || '').trim();
  await requireAdminPermission(req, action === 'automation:audit' ? 'ai:read' : 'ai:update');
  if (action === 'automation:audit' || action === 'automation:refresh') {
    const audit = await buildAutomationAudit(db, action === 'automation:refresh');
    await writeAuditLog(db, req, actor, action, 'automationRecommendations', 'automation', { label: 'Проверка автоматизаций', summary: audit.summary });
  ...
  if (action === 'automation:dismiss') {
    ...
    await writeAuditLog(db, req, actor, action, 'automationRecommendations', id, { label: `Отклонена автоматизация: ${recommendation.actionLabel || id}` });
  ...
  if (action === 'automation:confirm') {
    ...
    await writeAuditLog(db, req, actor, action, 'automationRecommendations', id, { label: `Подтверждена автоматизация: ${recommendation.actionLabel || id}`, result });
