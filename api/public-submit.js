...
export default async function handler(req, res) {
  ...
  try {
    ...
    const type = link.data.type;
    const draft = analyze(type, fields, files);
    const mediaManifest = buildMediaManifest(req.body?.files, files, req.body?.mediaSummary, draft.fields.videos);
    ...
    return res.status(200).json({ ok: true, id: requestRef.id, status: draft.status, missingFields: draft.missingFields, legalCheck, confidence: draft.confidence, mediaManifest });
  } catch (error) {
