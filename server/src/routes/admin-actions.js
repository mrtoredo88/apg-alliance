...
const RESOURCE_CONFIG = {
  ...
  conversationRequests: { collection: 'conversationRequests', scope: 'stats', label: 'заявка знакомства' },
  contextDialogs: { collection: 'contextDialogs', scope: 'stats', label: 'диалог' },
...
const LIST_CONFIG = {
  ...
  conversationRequests: { orderBy: ['createdAt', 'desc'], limit: 1000 },
  contextDialogs: { orderBy: ['updatedAt', 'desc'], limit: 1000 },
...
async function handleEconomyAction(db, request, actor) {
  ...
  await requireAdminPermission(request, action === 'economy:analytics' ? 'stats:read' : 'users:update');
  ...
  if (action === 'economy:backfill') {
    ...
    await writeAuditLog(db, request, actor, action, 'users', 'economy', { label: 'Backfill APG Economy 1.0', updated });
...
function buildPartnerReadiness(partner = {}) {
  ...
  const checks = [
    ...
    { key: 'links', label: 'Добавить хотя бы одну соцсеть или сайт', ok: Boolean(partner.websiteUrl || partner.vkGroupUrl || partner.vkUrl || partner.socialUrl || partner.telegramCommunityUrl || partner.telegramUrl || partner.maxCommunityUrl || partner.maxUrl), action: 'edit_links' },
    ...
    try {
      ...
      return action.startsWith('entity:')
        ...
        : action.startsWith('economy:')
          ? await handleEconomyAction(db, request, actor)
