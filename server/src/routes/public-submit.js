...
export default async function publicSubmitRoutes(fastify) {
  ...
  fastify.post('/api/public-submit', async (request, reply) => {
    ...
    try {
      ...
      if (action === 'track-partnership') {
        await writePartnershipAnalytics(db, cleanText(request.body?.event, 80), request.body);
        return { ok: true };
      }
      if (action === 'partnership-submit') {
        return await handlePartnershipSubmit(db, request.body);
      }
