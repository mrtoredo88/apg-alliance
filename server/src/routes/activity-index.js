...
export default async function activityIndexRoutes(fastify) {
  fastify.post('/api/activity-index', async (request, reply) => {
    try {
      ...
      if (isFirstOfMonth || request.body?.forceAward) {
      ...
      return { updated: results.length, month: monthKey, top3, monthStart: monthStartResult };
    } catch (e) {
