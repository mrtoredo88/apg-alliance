...
Основная production-платформа — PWA на `myapg.ru`; её основной пользовательский способ входа — Email OTP. Telegram и VK используются как дополнительные способы входа и каналы доступа.

### VK Mini App (дополнительный канал)
...
Native APG-клиент хранит срок сессии, обновляет её до истечения и выполняет единый дедуплицированный refresh при первом `401`. Защищённые `userAction`, включая каналы Локи, после успешной ротации повторяются ровно один раз; `403` не повторяется, чтобы не маскировать ошибки прав. APG Health показывает только сроки и события refresh/401, но никогда не сами токены.
