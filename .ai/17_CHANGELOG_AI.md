...
**Тип:** feat
**Что изменено:** раздел «Пользователи» получил поиск по идентификаторам, confidence-группы дублей, preview и безопасное объединение, массовое редактирование, архив/восстановление и защищённое удаление. Backend переносит связанные записи, сохраняет прогресс, архивирует aliases и пишет каждое действие в `adminActivity`.
**Почему:** администратору нужен единый понятный процесс окончательного разбора дублирующихся аккаунтов без потери пользовательских данных и с возможностью аудита.
...
**Тип:** feat
**Что изменено:** desktop-профиль получил цельный персональный hero, крупный аватар, баланс и прогресс, спокойную сетку ключевых метрик и облегчённую панель действий. Аватар теперь нормализуется из `photo_200`, общих avatar/photo полей и `linkedTelegram.photo`, а успешная привязка Telegram сразу обновляет фотографию текущего пользователя.
**Почему:** прежний экран выглядел как набор одинаковых белых плиток, а email-профили с привязанным Telegram не показывали фотографию, если она хранилась только внутри `linkedTelegram`.
...
**Что изменено:** Telegram auth пишет `status: done` до необязательных outbound-вызовов аватара и bot reply. Необязательные Telegram Bot API calls получили timeout 3.5 секунды, `getUpdates` — до трёх попыток по 7 секунд; polling сохраняет actionable error code. Production contract остаётся `POLLING_PRIMARY` с пустым webhook.
...
**Файлы:** `src/ProfilePanel.jsx`, `src/UserApp.jsx`, `scripts/people-navigation-test.mjs`, `package.json`, `.ai/17_CHANGELOG_AI.md`
**Тип:** fix
**Что изменено:**
...
# 2026-07-22 — Постоянная аватарка Telegram в PWA и desktop

- Исправлен источник аватарки профиля: backend загружает актуальное фото через Telegram Bot API и копирует его в постоянное публичное хранилище АПГ без раскрытия bot token клиенту.
- Добавлено авторизованное действие `telegramAvatar:refresh`, которое восстанавливает фото у уже привязанных Telegram-аккаунтов и сохраняет `photo`, `photo_200` и поля `linkedTelegram` в каноническом профиле.
- PWA и desktop-профиль автоматически вызывают восстановление, когда Telegram привязан, а фото отсутствует; полученное изображение появляется сразу без повторного входа.
- Устаревшие прямые Telegram file URL с bot token больше не используются для отображения; новый вход через Telegram сразу сохраняет безопасную постоянную ссылку.
- Общая загрузка в Yandex Object Storage вынесена в серверный helper и переиспользуется ручной загрузкой файлов и Telegram-аватарками.
- Регрессионные тесты профиля и Telegram avatar sync расширены проверками автоматического восстановления и отсутствия token-bearing URL.
# 2026-07-23 — Живой системный голос Локи

- Для PWA добавлен единый системный голосовой профиль Локи без внешнего TTS и дополнительных расходов.
- Выбор голоса теперь приоритизирует естественные русские женские голоса и premium/natural/neural-варианты устройства.
- Подача стала теплее и мультяшнее за счёт более живого темпа, повышенного тона, эмоциональных профилей и очистки Markdown, ссылок и emoji перед озвучкой.
- `LokiExperience`, карманный ассистент и общий `VoiceEngine` переведены на один helper, чтобы Локи звучал одинаково во всех интерфейсах.
- Регрессионный тест защищает выбор голоса и подготовку текста к произнесению.
# 2026-07-23 — Приватный архив демо-партнёра

- `Демо-партнёр АПГ` переведён в модель приватного архива: карточка, связанная новость и событие исключаются из публичной выдачи.
- Для карточки добавлены признаки `privateDemoAccess` и `demoVisibility: owners`; отдельное чтение по привязке кабинета сохраняет доступ только подтверждённым владельцам.
...
# 2026-07-26 — Android APK в окне установки

- В существующее мобильное окно добавлен приоритетный Android-вариант загрузки подписанного APK без создания нового модального окна.
...
# 2026-07-27 — Единая PWA-установка на всех телефонах

- Стартовое окно больше не предлагает Android-пакет и не содержит переходов на загрузку файла; Android и iPhone используют только установку веб-приложения.
- На Android используется системное окно `beforeinstallprompt`, а при его отсутствии показывается инструкция «Добавить на главный экран».
...
# 2026-07-28 — Профиль, уникальные посещения и новостная лента партнёра

- В профиле восстановлены постоянно доступные «Уведомления» и ролевой переход в «Кабинет партнёра».
- «Рефералы» переименованы в «Поделиться» без изменения перехода и механики приглашений.
- Удалена отдельная кнопка-стрелка в правом верхнем углу профиля.
- `profile:sync` больше не начисляет ключ за запуск приложения; награда остаётся только у подтверждённого визита, защищённого idempotency key и журналом уникальных visit rewards.
- В кабинете партнёра «Контент» переименован в «Новостная лента»; редактор открывается поверх интерфейса, закрывается по крестику, Escape и тапу по фону, блокирует прокрутку страницы и сохраняет существующий recovery черновика.
- Добавлены регрессионные тесты профиля, идемпотентности визита и модального редактора.

# 2026-07-28 — Восстановление Admin access и Desktop Workspace

- Native APG admin login теперь создаёт сессию по каноническому PostgreSQL Identity user id, а не по устаревшему Firebase UID.
- Проверка owner/admin роли выполняется по объединённой канонической записи; legacy `firebaseUid/authUid` сохранены только для поиска ранее созданного password credential.
...
# 2026-07-29 — Partner post, push re-login and connections reliability

- В кабинете партнёра раздел «Контент» переименован в «Создать пост» и сразу открывает модальный редактор.
- В редактор добавлены форматирование, фото, ссылки YouTube/VK Видео/Rutube, предпросмотр, календарная дата и действия «Отмена», «Черновик», «Опубликовать».
- Повторная авторизация больше не запрашивает push, если уведомления уже включены; ошибки permission, Service Worker и поддержки браузера объясняются отдельно.
- Исправлена запись запросов дружбы для объединённых и alias-аккаунтов: PostgreSQL получает реальный `user_id`, существующий во внешнем ключе, а канонический ID используется для защиты от знакомства с собственным alias.
- Добавлен регрессионный тест `test:partner-post-push-connections`, обновлён контракт notification delivery.
# 2026-07-29 — Safe actions in post editor and editable personal name

- Нижние действия редактора публикации подняты над нижним островом навигации и учитывают `safe-area-inset-bottom`.
- Имя в мобильном и desktop-профиле стало интерактивным: открывается форма редактирования имени, фамилии и даты рождения.
- `profile:update` сохраняет `birthDate` и отклоняет невалидную или будущую дату.
## 2026-07-29 — fix: ежедневный ключ больше не откатывается

- Синхронизация существующего профиля больше не переносит устаревшие поля экономики из Firestore в Account Core.
- Повторный запрос ежедневного бонуса безопасно восстанавливает ровно один потерянный ключ, если после начисления не было других операций.
- Добавлены регрессионные проверки защиты баланса и восстановления начисления.
## 2026-07-29 — ux: заметное редактирование имени в мобильном профиле

- Вся строка имени стала крупной кнопкой с отдельным значком редактирования.
- По нажатию открывается форма имени, фамилии и даты рождения.
- Добавлен стабильный UI-контракт и регрессионная проверка мобильной зоны нажатия.
## 2026-07-29 — fix: профиль, баланс ключей и массовое удаление

- Пользовательское имя теперь помечается как отредактированное и больше не заменяется старыми данными способа входа.
- Account Core сверяет текущий баланс с последней завершённой операцией и восстанавливает потерянное начисление.
- Owner может удалить выбранные профили из активного списка или архива; текстовое подтверждение, причина и блокировка связанных аккаунтов сохранены.
## 2026-07-29 — fix: запись профиля при расхождении canonical ID

- Редактирование профиля разрешает canonical ID, но сохраняет данные в фактическую Account Core-запись и identity пользователя сессии.
- История ключей сначала сверяет баланс с последней завершённой операцией, затем возвращает актуальный профиль.
- Production-диагностика выявила профиль `email:mrtoredo88@mail.ru`: подтверждённая операция имела баланс 33 при сохранённом балансе 32; баланс восстановлен до 33.
## 2026-07-30 — fix: надёжное мобильное нажатие на имя

- Вся строка с аватаром, именем и карандашом покрыта отдельной нативной кнопкой поверх декоративных слоёв.
- Декоративный фон карточки исключён из обработки касаний.
- Добавлена регрессионная проверка полной мобильной зоны редактирования профиля.

## 2026-07-30 — fix: единый доступный редактор профиля

- Невидимый абсолютный слой мобильной карточки заменён одной нативной кнопкой, которая включает аватар, имя и карандаш и не зависит от `z-index` или `pointer-events`.
- Исправлена первопричина: портал редактора отсутствовал в активной мобильной V2-ветке профиля, поэтому состояние менялось, но модальное окно не могло отрисоваться.
- Имя и карандаш desktop-профиля открывают тот же редактор и получили отдельные стабильные тестовые селекторы.
- Для обоих вариантов добавлены видимые hover, focus и pressed-состояния, клавиатурная доступность и доступное сообщение об ошибке сохранения.

## 2026-07-30 — fix: сохранение собственного профиля после объединения аккаунтов

- Редактор собственного профиля больше не отправляет устаревший или alias `userId`: сервер определяет владельца по подтверждённой identity-сессии.
- Защита от изменения чужого профиля сохранена для действий, где целевой пользователь передаётся явно.
- Серверное сохранение профиля теперь включает все поля формы: описание, телефон, Telegram и VK вместе с именем и датой рождения.

## 2026-08-01 — P0: самовосстановление Identity-сессии
