...
- Automation Engine генерирует рекомендации для событий платформы;
- поддержаны события: создан партнёр, создан эксперт, создано мероприятие, создана новость, создана акция, зарегистрирован пользователь, создан приз, завершено мероприятие;
- рекомендации сохраняются в `automationRecommendations`;
- подтверждение создаёт только черновики в существующих коллекциях или отмечает рекомендацию выполненной;
- автоматическая публикация не выполняется;
- Локи не является зависимостью Automation Engine, но сможет использовать рекомендации как источник будущих действий.

## APG Economy

Ключевые файлы:

- `server-shared/economy-engine.js`
- `src/economyEngine.js`
- `src/RewardsPage.jsx`
- `api/user-actions.js`
...
- единый тип валюты — ключи;
- билеты как отдельный баланс для розыгрышей;
- обмен ключей на билеты через централизованную конфигурацию;
- розыгрыши используют только `tickets`;
- магазин возможностей поддерживает будущие типы наград;
- репутация не тратится и определяет статус пользователя;
- QR, чтение новостей, комментарии, отзывы, задания, рефералы и посещения начисляются через Economy Engine;
- admin backend предоставляет аналитику экономики и безопасный backfill пользователей.

Граница ответственности:

- UI не начисляет и не списывает экономику напрямую;
- существующие `users.keys` сохраняются;
- новые поля `tickets`, `reputation`, `reputationStatus`, `economyVersion` добавляются совместимо со старыми пользователями.

## APG Learning System

Ключевые файлы:

- `src/learningSystem.js`
- `src/Onboarding.jsx`
- `src/ReferencePage.jsx`
- `src/TasksPage.jsx`
- `src/LokiPage.jsx`
- `src/loki/core/LokiCore.js`
- `src/UserApp.jsx`
...
- 7 коротких экранов первого запуска;
- учебные задания через существующую систему задач и начисление ключей через `task:claim`;
- центр знаний с аудиториями `Пользователи`, `Партнёры`, `Эксперты`, поиском, категориями, видео-текстами и пошаговыми инструкциями;
- контекстные одноразовые подсказки на ключевых экранах;
- повторный запуск обучения из профиля;
- Локи объясняет текущий экран по `activePanel`;
- прогресс обучения сохраняется в `users.learningProgress`, а аналитика прохождения — в `users.learningAnalytics`.

Граница ответственности:

- Learning System не создаёт отдельную систему наград;
- начисление ключей остаётся backend-first через существующий `task:claim`;
- Локи использует текущий Context/App State и не читает данные обучения из параллельного источника.
...
Подтверждённая роль: основная production-платформа АПГ на `https://myapg.ru`.
...
Подтверждённая роль: дополнительный канал к основной PWA-платформе.
