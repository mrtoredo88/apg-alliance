...
Мобильный модуль контента кабинета использует actions `workspaceNews:list`, `workspaceNews:save`, `workspaceNews:submit`, `workspaceNews:archive`, `workspaceNews:fromEvent`. Backend проверяет владение профилем через `ownerId`/привязку пользователя. `workspaceNews:save` публикует материал в личную ленту профиля (`distributionMode: profile`, `profileOnly: true`), а `workspaceNews:submit` переводит материал в режим общей ленты АПГ (`distributionMode: apg`) со статусом `moderation`; публичная публикация остаётся за редактором/модерацией.
...
**Production runtime:** возвращает `runtime.backend` (`git`, `image`, `build`, `appVersion`, `runtime`) и `telegramAuth` (`deliveryMode`, `lastPollAt`, `pollAgeSec`, `lastError`, `lastErrorCode`, `pendingSessions`, `lastAuthDoneAt`, `lastAuthFailureReason`). Последние Telegram auth sessions доступны в `telegramAuthSessions.recent` без чувствительных токенов.
...
- `review:partner`, `review:expert` — отзыв; при переданном `bookingId` закрывает review-step связанной встречи
- `booking:create` — создать встречу со статусом `pending`, зеркала пользователя/владельца, контекстный диалог и уведомление
- `booking:confirm` — подтверждение встречи владельцем партнёра/эксперта
- `booking:cancel` — отмена встречи пользователем или владельцем с сохранением причины и истории
- `booking:requestReschedule` — запрос переноса встречи с временной блокировкой нового слота
- `booking:respondReschedule` — принятие или отклонение переноса владельцем
- `booking:complete`, `booking:noShow` — завершение встречи или отметка неявки; завершение идемпотентно запускает post-visit journey: ключи/репутация по economy rules, визит/штамп, activity, диалог и prompt отзыва
- `booking:list`, `booking:calendar` — список и календарное представление встреч для профиля пользователя или владельца
- `booking:moment` — аналитика экрана “Спасибо за визит”: `opened`, `review_started`, `review_submitted`, `dialog_clicked`, `rebook_clicked`, `dismissed`
...
Возвращает защищённую историю операций с ключами текущего пользователя и авторитетный серверный баланс. Читает `users/{userId}/activity`, принимает `limit` от 1 до 200. Для каждой операции возвращает тип, дату, причину, источник, signed-изменение баланса, расчётный итоговый баланс и статус. Запрос другого пользователя блокируется через `assertOwn`.
...
{
  "state": "a1b2c3d4e5f6...",
  "url": "https://telegram.me/apg_zelenograd_bot?start=auth_a1b2c3d4e5f6..."
}
```
...
Production delivery — `POLLING_PRIMARY`: auth-check вызывает `getUpdates`, а timer подхватывает органические
команды. Webhook должен быть пустым. `status: done` записывается до best-effort загрузки Telegram-аватара,
чтобы сбой outbound Telegram API не блокировал Firebase custom-token flow.
