...
**Логика:** `wall.get` для `owner_id: -229980067`, `filter=owner`. Посты без текста и вложений пропускаются. Все фото, видео, ссылки, документы, хэштеги и показатели активности нормализуются в структуру новости АПГ. VK-посты получают `category: "apg"`, поэтому отображаются в категории «Новости АПГ», а переход во ВКонтакте остаётся дополнительной кнопкой внизу статьи.
