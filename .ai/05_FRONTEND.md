...
- профиль пользователя показывает блок `Мои записи`;
- кабинет партнёра/эксперта содержит модуль `Запись` для включения записи и базовых слотов;
- серверное действие `booking:create` создаёт `bookings/{bookingId}`, зеркала в `users/{id}/bookings` и контекстный диалог `type: booking`.
