...
На desktop левый sidebar всегда показывает полный список рабочих разделов. Мобильная группировка навигации не используется в Desktop Workspace: CRM/встречи, диалоги, мероприятия, публикации, акции, подарки, аналитика и настройки доступны напрямую.
...
`src/loki/lokiVoice.js` задаёт единый системный голосовой профиль Локи без внешнего TTS: выбирает наиболее естественный русский голос устройства, очищает Markdown, ссылки и emoji перед произнесением и применяет живую мультяшную подачу через темп, высоту и эмоциональные профили. `LokiExperience`, карманный ассистент и `VoiceEngine` используют один helper, поэтому звучание одинаково во всех поверхностях. Фактический тембр зависит от установленных в ОС голосов; при отсутствии русского голоса браузер использует системный fallback.

Архивный `demo-partner-apg` не входит в публичный массив партнёров. Его владельцы продолжают получать карточку отдельным чтением по `user.partnerId` / `partnerCabinetIds`, если на карточке установлены `privateDemoAccess: true` и `demoVisibility: owners`. Остальные архивные партнёры этим исключением не затрагиваются.
