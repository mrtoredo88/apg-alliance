...
- Публичная форма не требует авторизации пользователя; состоит из шага публичной карточки и шага юридической карточки. После отправки backend создаёт заявку в `aiImportRequests` и закрывает ссылку от повторной отправки
- Юридическая карточка хранится отдельно от публичного черновика: `legalProfile`, `legalDocuments`, `legalCheck`, `counterparty`, CRM-заготовка под договоры, счета, акты, ЭДО и комментарии
