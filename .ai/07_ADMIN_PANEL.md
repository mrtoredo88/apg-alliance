...
**Авторизация:** вход из профиля виден для `admin/owner` и legacy VK id; внутри админки пока остаётся `PasswordGate` без серверной проверки роли  
...
- комментарии новостей читаются из `newsComments` и модерируются через `/api/news-comments`
...
- Пустой раздел-заготовка для V4.5
- Предусмотрены области: черновики материалов, источники и факты, очередь публикаций, проверка редактором
