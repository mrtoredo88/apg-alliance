...
- Единое рабочее место с представлениями `Активные`, `Вероятные дубли`, `Архив`
- Поиск по имени, email, телефону, Telegram, Firebase UID и id аккаунта
- Поиск дублей учитывает точные совпадения email/телефона/Telegram/Firebase UID и похожесть нормализованного имени; группы ранжируются по confidence
- Разбор группы начинается с выбора основного аккаунта и обязательного preview: показываются итоговые ключи/билеты/aliases и количество переносимых связей
- Объединение сохраняет массивы прогресса, суммирует счётчики и карты посещений, переносит найденные ссылки, а исходные аккаунты архивирует как `legacy_linked` с `mergedInto`
- Доступны редактирование профиля, выбор нескольких строк, массовое обновление, архивирование и восстановление
- Окончательное удаление доступно только `owner`, требует текстового подтверждения и блокируется при наличии связанных записей
- Все операции выполняются через `/api/admin-actions` с role guard, идемпотентностью для merge/delete и записью в `adminActivity`
