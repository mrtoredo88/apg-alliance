...
const TARIFFS = [
  ['start', 'Старт', 'Основная карточка эксперта'],
  ['standard', 'Стандарт', 'Расширенная публичная карточка'],
  ['premium', 'Премиум', 'Новости, мероприятия и юридические данные'],
  ['ambassador', 'Амбассадор', 'Максимальные возможности и участие в развитии АПГ'],
];
const FORMAT_OPTIONS = [...EXPERT_WORK_FORMATS, { id: 'remote', label: 'Дистанционно' }];
...
export function ExpertQuestionnaire({ fields, files, onField, onFiles, uploading }) {
  const [open, setOpen] = useState({ main: true, services: true, offer: false, media: false, tariff: true, contacts: false, legal: false, photos: false, comment: false });
  const [videoInput, setVideoInput] = useState('');
  const [socialInput, setSocialInput] = useState('');
  const [videoError, setVideoError] = useState('');
  const premium = hasPremiumExpertAccess(fields.tariff);
  ...
  return <>
    ...
    <Section id="main" title="1. Основная информация" subtitle="Имя, направления и профессиональное описание" open={open.main} onToggle={toggle}>
    ...
    <Section id="services" title="2. Услуги" subtitle="Что вы предлагаете и в каких форматах работаете" open={open.services} onToggle={toggle}>
    ...
    <Section id="offer" title="3. Специальное предложение" subtitle="Акция только для пользователей АПГ" open={open.offer} onToggle={toggle}><Field label="Акция для пользователей АПГ" hint="Скидка, подарок, бесплатная консультация, бонус или промокод."><textarea style={textareaStyle()} value={fields.offer || ''} onChange={e => onField('offer', e.target.value)} /></Field></Section>

    <Section id="media" title="4. Медиа" subtitle="Добавьте 1–3 публичных видео" open={open.media} onToggle={toggle}>
    ...
    <Section id="tariff" title="5. Тариф" subtitle="Состав анкеты меняется автоматически" open={open.tariff} onToggle={toggle}><div style={{ display: 'grid', gap: 8 }}>{TARIFFS.map(([id, label, description]) => <button key={id} type="button" onClick={() => onField('tariff', id)} style={{ padding: 12, borderRadius: 15, border: `1px solid ${fields.tariff === id ? 'rgba(201,168,76,0.52)' : 'rgba(25,23,19,0.10)'}`, background: fields.tariff === id ? 'rgba(201,168,76,0.16)' : 'rgba(255,255,255,0.52)', textAlign: 'left', cursor: 'pointer' }}><strong>{label}</strong><span style={{ display: 'block', marginTop: 3, fontSize: 11, color: 'rgba(25,23,19,0.56)' }}>{description}</span></button>)}</div>{premium && <div style={{ marginTop: 12 }}><Field label="Новости" hint="Темы и поводы, о которых можно рассказать пользователям."><textarea style={textareaStyle()} value={fields.newsInfo || ''} onChange={e => onField('newsInfo', e.target.value)} /></Field><Field label="Мероприятия" hint="Встречи, эфиры, консультации и другие активности."><textarea style={textareaStyle()} value={fields.activities || ''} onChange={e => onField('activities', e.target.value)} /></Field></div>}</Section>
    ...
    {premium && <Section id="legal" title="7. Юридические данные" subtitle="Доступно для Премиум и Амбассадор" open={open.legal} onToggle={toggle}><Field label="ИНН" hint="Другие реквизиты появятся позднее в защищённом юридическом профиле." error={fieldErrors.inn}><input style={inputStyle(!!fieldErrors.inn)} value={fields.inn || ''} onChange={e => onField('inn', e.target.value.replace(/\D/g, '').slice(0, 12))} /></Field></Section>}

    <Section id="photos" title="8. Фотографии" subtitle="Аватар, логотип, галерея и горизонтальная обложка" open={open.photos} onToggle={toggle}>
      {[['avatar', 'Аватар', 'Квадратное фото от 800×800 px, 1 файл'], ['logo', 'Логотип — если используется', 'PNG или WebP с прозрачным фоном, 1 файл'], ['gallery', 'Фотографии', 'Рекомендуется 3–6 вертикальных или квадратных фото'], ['cover', 'Горизонтальная обложка карточки', 'Рекомендуемый размер от 1600×900 px, 1 файл']].map(([role, label, hint]) => <Field key={role} label={label} hint={hint}><input type="file" accept="image/*" multiple={role === 'gallery'} onChange={e => onFiles(e.target.files, role)} style={{ ...inputStyle(), padding: 9 }} /></Field>)}
