...
export function LokiLogoButton({ onClick, size = 44, radius = 18, style }) {
  const openLokiDialog = (event) => {
    event.preventDefault();
    event.stopPropagation();
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('apg:open-loki-dialog'));
      return;
    }
    onClick?.(event);
  };
  ...
  return (
    <button type="button" onClick={openLokiDialog} aria-label="Открыть диалог с Локи" title="Открыть диалог с Локи" style={{ width: size, height: size, padding: 0, border: 0, borderRadius: radius, background: 'transparent', position: 'relative', display: 'grid', placeItems: 'center', flexShrink: 0, cursor: 'pointer', isolation: 'isolate', ...style }}>
