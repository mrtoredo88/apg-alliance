...
import { 
  ...
  Tabbar, TabbarItem, Placeholder, Epic
} from '@vkontakte/vkui';
...
export const AppConfig = () => {
  // 1. Создаем переменную состояния для переключения экранов
  const [activePanel, setActivePanel] = useState('profile');

  // 2. Функция для открытия сканера QR-кода
  const openScanner = () => {
    vkBridge.send('VKWebAppOpenQR')
      .then((data) => {
        if (data.qr_data) {
          alert('Код успешно отсканирован: ' + data.qr_data);
          // Здесь позже добавим логику начисления ключей
        }
      })
      .catch((error) => {
      ...
      <AdaptivityProvider>
        <AppRoot>
          <Epic activeStory={activePanel} tabbar={
            <Tabbar>
              <TabbarItem 
                onClick={() => setActivePanel('home')} // Переключаем на главную
              ...
              <TabbarItem 
                onClick={openScanner} // Вызываем камеру
                text="Сканировать"
                style={{ backgroundColor: 'var(--vkui--color_background_accent_themed)', borderRadius: '8px' }}
              >
                <Icon28QrCodeOutline fill="white" />
              </TabbarItem>
              
              <TabbarItem 
                onClick={() => setActivePanel('profile')} // Переключаем на профиль
