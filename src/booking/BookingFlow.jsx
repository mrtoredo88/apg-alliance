...
export function BookingFlow({ open, provider, providerType = 'partner', user, onClose, onCreated, onOpenDialog }) {
  const bookingProfile = useMemo(() => buildBookingProfile(provider || {}, providerType), [provider, providerType]);
  ...
  const slots = activeDate ? buildBookingSlots({ date: activeDate, service, specialist, profile: provider }) : [];
  ...
  useEffect(() => {
    ...
    setComment('');
    setError('');
  }, [bookingProfile.providerId, bookingProfile.providerType, open]);
      ...
      <GlassPanel style={{ minHeight: '100svh', maxWidth: 560, margin: '0 auto', background: 'transparent', paddingBottom: 'calc(36px + env(safe-area-inset-bottom, 0px))' }}>
        ...
        <GlassCard style={{ borderRadius: 32, display: 'flex', gap: 13, alignItems: 'center', marginBottom: 14 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ color: APG2_PROFILE.textMuted, fontSize: 12.5, lineHeight: '18px', marginTop: 4 }}>{bookingProfile.subtitle || bookingProfile.address || 'Выберите услугу и свободное время'}</div>
          </div>
        ...
        <GlassSection title="Подтверждение">
          <GlassCard style={{ borderRadius: 28, display: 'grid', gap: 9 }}>
            ...
            <div style={{ color: APG2_PROFILE.textSoft, fontSize: 13, lineHeight: '19px' }}>{specialist?.name || 'Специалист'}{slot ? ` · ${formatFullDate(slot.startAt)} · ${slot.time}` : ''}</div>
