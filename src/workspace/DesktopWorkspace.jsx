...
function Panel({ title, action, children, style }) {
  return (
    <section style={cardStyle({ padding: 24, minWidth: 0, ...style })}>
...
function WorkspaceHeader({ query, onQueryChange, unreadCount, onModeChange, onOpenNotifications }) {
  ...
  return (
    <header data-workspace-v2-header style={{ position: 'sticky', top: 0, zIndex: 30, background: 'rgba(255,249,238,0.86)', backdropFilter: 'blur(28px) saturate(1.25)', WebkitBackdropFilter: 'blur(28px) saturate(1.25)', borderBottom: `1px solid ${WS.line}` }}>
      <div style={{ maxWidth: 1760, margin: '0 auto', minHeight: 84, padding: '0 26px', display: 'grid', gridTemplateColumns: '290px minmax(0,1fr) auto', gap: 22, alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, minWidth: 0 }}>
          <div style={{ width: 58, height: 58, borderRadius: 17, background: 'linear-gradient(135deg,#23145D,#0E0B28)', display: 'grid', placeItems: 'center', color: '#F3D583', fontWeight: 950, boxShadow: '0 14px 30px rgba(47,28,105,0.22)' }}>АПГ</div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 10 }}>
          <label style={{ width: 270, minHeight: 48, borderRadius: 20, background: 'rgba(255,255,255,0.78)', border: `1px solid ${WS.line}`, display: 'flex', alignItems: 'center', gap: 10, padding: '0 14px', boxSizing: 'border-box', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.86)' }}>
          ...
          <button type="button" onClick={onOpenNotifications} style={buttonStyle({ width: 52, minHeight: 52, padding: 0, borderRadius: 18, position: 'relative', fontSize: 21 })}>
            ♧
          ...
          <WorkspaceButton onClick={() => onModeChange?.('workspace')} style={{ minHeight: 52, borderRadius: 20, padding: '0 22px', background: 'linear-gradient(135deg,#F6D891,#D0A14C)', color: '#24190B', boxShadow: '0 14px 30px rgba(201,155,60,0.22)' }}>▣ Workspace</WorkspaceButton>
        </div>
      </div>
    </header>
  );
...
function WorkspaceSidebar({ items, activeSection, onSelect, user, data, onModeChange }) {
  ...
  return (
    <aside data-workspace-v2-sidebar style={cardStyle({ height: 'calc(100dvh - 128px)', minHeight: 760, position: 'sticky', top: 104, padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column' })}>
        ...
        {[main, settings].map((group, groupIndex) => (
          <div key={groupIndex} style={{ display: 'grid', gap: 2 }}>
            {group.map(item => {
              ...
              return (
                <button key={`${item.id}-${item.label}`} type="button" onClick={() => onSelect(item)} style={{ border: 0, minHeight: 54, padding: '0 24px', background: active ? 'linear-gradient(90deg, rgba(241,206,128,0.42), rgba(241,206,128,0.08))' : 'transparent', color: active ? '#8A6422' : WS.text, display: 'grid', gridTemplateColumns: '28px minmax(0,1fr) auto', alignItems: 'center', gap: 12, cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left', boxShadow: active ? 'inset 4px 0 0 #D0A14C' : 'none' }}>
            ...
            {groupIndex === 0 && <div style={{ height: 12 }} />}
          </div>
        ))}
      </div>
      <div style={{ marginTop: 'auto', padding: 24, display: 'grid', gap: 18 }}>
        <div style={cardStyle({ padding: 14, borderRadius: 18, background: 'rgba(255,252,245,0.86)', boxShadow: 'none' })}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 48, height: 48, borderRadius: 16, background: 'linear-gradient(135deg,#100B32,#4A327F)', color: '#F5D77E', display: 'grid', placeItems: 'center', fontWeight: 950 }}>{initial}</div>
            <div style={{ minWidth: 0 }}>
        ...
        <WorkspaceButton onClick={() => onModeChange?.('user')} style={{ minHeight: 58, borderRadius: 18, background: 'rgba(255,255,255,0.82)' }}>☷ Перейти в режим пользователя</WorkspaceButton>
      </div>
    </aside>
  );
}

function Sparkline({ color = WS.gold }) {
  return (
    <svg viewBox="0 0 520 180" preserveAspectRatio="none" style={{ width: '100%', height: 192, display: 'block' }}>
      <defs>
      ...
      <div style={{ position: 'relative', zIndex: 1, display: 'grid', height: '100%', minHeight: 360, alignContent: 'space-between', gap: 28 }}>
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 18, alignItems: 'flex-start' }}>
            ...
            <div style={{ borderRadius: 18, background: 'rgba(255,255,255,0.13)', border: '1px solid rgba(255,255,255,0.12)', padding: '11px 16px', color: '#F5D77E', fontSize: 14, fontWeight: 850, backdropFilter: 'blur(18px)' }}>♕ Партнёр с 22 мая 2024</div>
          </div>
          <h1 style={{ margin: '20px 0 0', maxWidth: 620, color: '#fff', fontSize: 38, lineHeight: '47px', fontWeight: 950, letterSpacing: -0.75 }}>Ваши партнёрские возможности на сегодня</h1>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0,1fr))', gap: 16, padding: 18, borderRadius: 24, background: 'rgba(10,10,12,0.42)', border: '1px solid rgba(255,255,255,0.08)', backdropFilter: 'blur(26px)' }}>
          {[
            ['☆', '18', 'Уровень'],
            ...
            <div key={label} style={{ display: 'grid', gridTemplateColumns: '42px minmax(0,1fr)', gap: 12, alignItems: 'center' }}>
              <div style={{ width: 42, height: 42, borderRadius: 14, background: 'rgba(201,155,60,0.14)', color: '#F2C963', display: 'grid', placeItems: 'center', fontSize: 21 }}>{icon}</div>
              <div>
                <div style={{ color: '#fff', fontSize: 25, lineHeight: '28px', fontWeight: 930 }}>{value}</div>
...
function MetricTile({ label, value, delta }) {
  return (
    <div style={{ borderRadius: 16, background: 'rgba(255,255,255,0.78)', border: `1px solid ${WS.line}`, padding: '18px 14px', textAlign: 'center', minWidth: 0 }}>
      <div style={{ color: WS.text, fontSize: 26, lineHeight: '30px', fontWeight: 950 }}>{value}</div>
      <div style={{ color: WS.soft, fontSize: 12.5, lineHeight: '16px', marginTop: 5 }}>{label}</div>
      <div style={{ color: WS.green, fontSize: 12.5, lineHeight: '16px', fontWeight: 880, marginTop: 9 }}>↗ {delta}</div>
    </div>
  );
}
...
function MetricsPanel({ data }) {
  return (
    <Panel title="Ключевые показатели" action={<button type="button" style={buttonStyle({ minHeight: 34, padding: '7px 12px', background: 'transparent', boxShadow: 'none', color: WS.soft })}>Этот месяц⌄</button>} style={{ minHeight: 430, padding: 26 }}>
...
function TaskRow({ icon, title, text, priority, tone = WS.gold, onClick }) {
  return (
    <button type="button" onClick={onClick} style={{ border: 0, background: 'transparent', padding: '14px 0', borderBottom: `1px solid ${WS.line}`, display: 'grid', gridTemplateColumns: '46px minmax(0,1fr) auto', gap: 14, alignItems: 'center', textAlign: 'left', cursor: 'pointer', fontFamily: 'inherit' }}>
      <span style={{ width: 44, height: 44, borderRadius: 14, background: `${tone}18`, color: tone, display: 'grid', placeItems: 'center', fontSize: 20 }}>{icon}</span>
...
function EventRow({ event, index, onClick }) {
  ...
  return (
    <button type="button" onClick={onClick} style={{ border: 0, background: 'transparent', padding: '13px 0', borderBottom: `1px solid ${WS.line}`, display: 'grid', gridTemplateColumns: '74px minmax(0,1fr) auto', gap: 16, alignItems: 'center', textAlign: 'left', cursor: 'pointer', fontFamily: 'inherit' }}>
      <span style={{ width: 74, height: 74, borderRadius: 18, background: index === 0 ? 'rgba(201,155,60,0.16)' : 'rgba(88,67,37,0.06)', display: 'grid', placeItems: 'center', color: WS.text }}>
        <span style={{ textAlign: 'center' }}>
          <span style={{ display: 'block', fontSize: 30, lineHeight: '30px', fontWeight: 950 }}>{date ? date.getDate() : '—'}</span>
...
function MessageRow({ item, index, onClick }) {
  return (
    <button type="button" onClick={onClick} style={{ border: 0, background: 'transparent', padding: '13px 0', borderBottom: `1px solid ${WS.line}`, display: 'grid', gridTemplateColumns: '54px minmax(0,1fr) auto', gap: 14, alignItems: 'center', textAlign: 'left', cursor: 'pointer', fontFamily: 'inherit' }}>
      <span style={{ width: 54, height: 54, borderRadius: 999, background: index === 0 ? 'linear-gradient(135deg,#1C173F,#8367C7)' : 'linear-gradient(135deg,#F1DCA4,#D0A14C)', display: 'grid', placeItems: 'center', color: '#fff', fontWeight: 930 }}>{String(safeTitle(item, 'АПГ')).slice(0, 1).toUpperCase()}</span>
      <span style={{ minWidth: 0 }}>
...
function WorkspaceDashboard({ data, actions }) {
  ...
  const visibleEvents = data.events.length ? data.events.slice(0, 3) : fallbackEvents;
  const visibleMessages = data.notifications.length ? data.notifications.slice(0, 3) : [{ id: 'm1' }, { id: 'm2' }, { id: 'm3' }];

  return (
    <div data-workspace-v2-dashboard style={{ display: 'grid', gap: 18 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.12fr) minmax(430px,0.88fr)', gap: 18 }}>
        <DashboardHero data={data} profileStatus={profileStatus} />
        <MetricsPanel data={data} />
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,0.9fr) minmax(0,1.05fr) minmax(0,1fr)', gap: 18, alignItems: 'stretch' }}>
        <Panel title="Задачи и уведомления" action={<button type="button" style={buttonStyle({ minHeight: 34, padding: '7px 11px', background: 'transparent', boxShadow: 'none', color: WS.soft })}>Все задачи⌄</button>}>
          <div style={{ display: 'grid' }}>
            {tasks.map(task => <TaskRow key={task.title} {...task} />)}
          </div>
        </Panel>
        <Panel title="Ближайшие мероприятия" action={<button type="button" onClick={actions.openEvents} style={buttonStyle({ minHeight: 34, padding: '7px 11px', background: 'transparent', boxShadow: 'none', color: WS.soft })}>Все мероприятия⌄</button>}>
          <div style={{ display: 'grid' }}>
            {visibleEvents.map((event, index) => <EventRow key={event.id || index} event={event} index={index} onClick={actions.openEvents} />)}
          </div>
        </Panel>
        <Panel title="Новые сообщения" action={<button type="button" onClick={actions.openMessages} style={buttonStyle({ minHeight: 34, padding: '7px 11px', background: 'transparent', boxShadow: 'none', color: WS.soft })}>Все сообщения⌄</button>}>
          <div style={{ display: 'grid' }}>
            {visibleMessages.map((item, index) => <MessageRow key={item.id || index} item={item} index={index} onClick={actions.openMessages} />)}
          </div>
        </Panel>
      </div>
      <Panel title="Быстрые действия" style={{ padding: 22 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, minmax(0,1fr))', gap: 14 }}>
          {[
            ['✧', 'Создать акцию', 'Добавить новое предложение', actions.openOffers],
            ['▤', 'Опубликовать новость', 'Рассказать о событиях', actions.openNews],
            ['□', 'Добавить мероприятие', 'Запланировать событие', actions.openEvents],
            ['☷', 'Пригласить партнёра', 'Расширить сеть контактов', actions.openPartners],
            ['↗', 'Создать рассылку', 'Отправить сообщение', actions.openMessages],
          ].map(([icon, title, text, onClick]) => (
            <button key={title} type="button" onClick={onClick} style={{ border: `1px solid ${WS.line}`, borderRadius: 18, background: 'rgba(255,255,255,0.68)', padding: 16, minHeight: 86, display: 'grid', gridTemplateColumns: '48px minmax(0,1fr)', gap: 12, alignItems: 'center', textAlign: 'left', cursor: 'pointer', fontFamily: 'inherit' }}>
              <span style={{ width: 48, height: 48, borderRadius: 14, background: 'rgba(201,155,60,0.14)', color: WS.gold, display: 'grid', placeItems: 'center', fontSize: 21 }}>{icon}</span>
              <span style={{ minWidth: 0 }}>
                ...
                <span style={{ display: 'block', color: WS.soft, fontSize: 12.5, lineHeight: '16px', marginTop: 4, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{text}</span>
              </span>
            </button>
          ))}
        </div>
