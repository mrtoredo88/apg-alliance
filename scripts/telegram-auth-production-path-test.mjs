...
assert.match(webhook, /messages\?\.\[0\]\?\.details\?\.payload[\s\S]*JSON\.parse\(timerPayload\)\.secret/, 'timer authenticates the documented Yandex TimerMessage envelope');
...
assert.match(deploy, /execution-timeout 60s[\s\S]*TELEGRAM_WEBHOOK_SECRET[\s\S]*TELEGRAM_DELIVERY_MODE=background/, 'production combines fast warm-instance polling with the external timer watchdog');
