...
assert.match(economyRepository, /latestOperation/);
assert.match(economyRepository, /ORDER BY created_at DESC/);
assert.match(economyRepository, /balanceReconciled/);
assert.match(userActions, /accountProfileForSync/);
assert.match(userActions, /delete accountProfileForSync\[field\]/);
assert.match(userActions, /current\.profileNameEditedAt/);
assert.match(userActions, /patch\.profileNameSource = 'user'/);
assert.match(userActions, /accountProfile\?\.userId \|\| actor\.userId/);
assert.match(userActions, /writeIdentityProfileBestEffort\(identityUserId, patch\)/);
assert.match(userActions, /const rows = await serverFoundation\.account\.economyHistory/);
assert.match(economyRepository, /resolvedUserId/);
assert.match(economyRepository, /latestCompleted/);
